...
async def get_vendor_analytics(limit: int = 10, db: AsyncSession = Depends(get_db_session)):
...
async def get_trends(days: int = 30, db: AsyncSession = Depends(get_db_session)):
    service = AnalyticsService(db)
