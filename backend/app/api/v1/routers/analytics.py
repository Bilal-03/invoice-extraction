...
async def get_summary(db: AsyncSession = Depends(get_db_session), tenant_id: str = Depends(get_tenant_id)):
...
async def get_vendor_analytics(limit: int = 10, db: AsyncSession = Depends(get_db_session), tenant_id: str = Depends(get_tenant_id)):
...
async def get_trends(days: int = 30, db: AsyncSession = Depends(get_db_session), tenant_id: str = Depends(get_tenant_id)):
    service = AnalyticsService(db, tenant_id)
